CREATE TABLE IF NOT EXISTS `user_configuration`
(
    `id`                BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Internal id of the user configuration',
    `channel_id`        VARCHAR(30) NOT NULL UNIQUE COMMENT 'Twitch id of the user this configuration item is for',
    `se_jwt`            VARCHAR(1000) COMMENT 'StreamElements JWT token for subscribing to events',
    `follower_seconds`  BIGINT UNSIGNED DEFAULT 10 COMMENT 'Seconds to add per follower',
    `raider_seconds`    BIGINT UNSIGNED DEFAULT 1 COMMENT 'Seconds to add per raider',
    `tier_1_seconds`    BIGINT UNSIGNED DEFAULT 300 COMMENT 'Seconds to add per Tier 1 subscription',
    `tier_2_seconds`    BIGINT UNSIGNED DEFAULT 600 COMMENT 'Seconds to add per Tier 2 subscription',
    `tier_3_seconds`    BIGINT UNSIGNED DEFAULT 1500 COMMENT 'Seconds to add per Tier 3 subscription',
    `tier_1_gift_seconds` BIGINT UNSIGNED DEFAULT 300 COMMENT 'Seconds to add per Tier 1 gift subscription',
    `tier_2_gift_seconds` BIGINT UNSIGNED DEFAULT 600 COMMENT 'Seconds to add per Tier 2 gift subscription',
    `tier_3_gift_seconds` BIGINT UNSIGNED DEFAULT 1500 COMMENT 'Seconds to add per Tier 3 gift subscription',
    `currency_seconds`  BIGINT UNSIGNED DEFAULT 60 COMMENT 'Seconds to add per 1 currency donation',
    `bits_seconds`      BIGINT UNSIGNED DEFAULT 60 COMMENT 'Seconds to add per 100 bits',
    `initial_seconds`   BIGINT UNSIGNED DEFAULT 3600 COMMENT 'Initial seconds for timers',
    `insert_time`       TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT 'Insertion timestamp, auto generated by database',
    PRIMARY KEY (`id`)
);

CREATE TABLE IF NOT EXISTS `timer`
(
    `id`                UUID NOT NULL COMMENT 'Internal id of the timer',
    `channel_name`      VARCHAR(30) NOT NULL COMMENT 'Name of the Twitch channel',
    `channel_id`        VARCHAR(30) NOT NULL COMMENT 'Twitch id of the channel',
    `start_time`        TIMESTAMP COMMENT 'Start timestamp of the timer',
    `end_time`          TIMESTAMP COMMENT 'End timestamp of the timer',
    `state`             ENUM ('UNINITIALIZED', 'INITIALIZED', 'PAUSED', 'TICKING', 'ENDED') NOT NULL COMMENT 'Timer state',
    `update_time`       TIMESTAMP NOT NULL COMMENT 'Timestamp of the last update of the timer',
    `insert_time`       TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT 'Insertion timestamp, auto generated by database',
    PRIMARY KEY (`id`)
);

CREATE TABLE IF NOT EXISTS `event`
(
    `id`          BIGINT UNSIGNED AUTO_INCREMENT NOT NULL COMMENT 'Id of the event',
    `type`        ENUM ('FOLLOW', 'RAID', 'SUBSCRIPTION', 'GIFT', 'TIP', 'CHEER', 'COMMAND') NOT NULL COMMENT 'Type of the event',
    `timestamp`   TIMESTAMP NOT NULL COMMENT 'Timestamp of the event',
    `source`      VARCHAR(20) COMMENT 'Source of the event',
    `username`    VARCHAR(30) NOT NULL COMMENT 'Username of the person causing the event',
    `insert_time` TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT 'Insertion timestamp, auto generated by database',
    PRIMARY KEY (`id`)
);

CREATE TABLE IF NOT EXISTS `follow_event`
(
    `event_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Reference to the event',
    PRIMARY KEY (`event_id`),
    CONSTRAINT `fk_follower_event_event`
        FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
);

CREATE TABLE IF NOT EXISTS `raid_event`
(
    `event_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Reference to the event',
    `amount`   BIGINT NOT NULL COMMENT 'Amount of raiders brought in by the raid',
    PRIMARY KEY (`event_id`),
    CONSTRAINT `fk_raid_event_event`
        FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
);

CREATE TABLE IF NOT EXISTS `subscription_event`
(
    `event_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Reference to the event',
    `tier`     ENUM ('PRIME', 'TIER_1', 'TIER_2', 'TIER_3') NOT NULL COMMENT 'Subscription tier',
    `gifted`   BOOLEAN NOT NULL COMMENT 'If the subscription was gifted',
    `sender`   VARCHAR(30) COMMENT 'The sender in case of a gifted subscription',
    PRIMARY KEY (`event_id`),
    CONSTRAINT `fk_subscription_event_event`
        FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
);

CREATE TABLE IF NOT EXISTS `community_gift_event`
(
    `event_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Reference to the event',
    `amount`   BIGINT NOT NULL COMMENT 'Amount of gifted subs',
    `tier`     ENUM ('TIER_1', 'TIER_2', 'TIER_3') NOT NULL COMMENT 'Tier of the gifted subs',
    PRIMARY KEY (`event_id`),
    CONSTRAINT `fk_community_gift_event_event`
        FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
);

CREATE TABLE IF NOT EXISTS `tip_event`
(
    `event_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Reference to the event',
    `amount`   DOUBLE NOT NULL COMMENT 'Amount of money tipped',
    `currency` VARCHAR(30) COMMENT 'Currency of the tip',
    PRIMARY KEY (`event_id`),
    CONSTRAINT `fk_tip_event_event`
        FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
);

CREATE TABLE IF NOT EXISTS `cheer_event`
(
    `event_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Reference to the event',
    `amount`   BIGINT NOT NULL COMMENT 'Amount of bits cheered',
    PRIMARY KEY (`event_id`),
    CONSTRAINT `fk_cheer_event_event`
        FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
);

CREATE TABLE IF NOT EXISTS `command_event`
(
    `event_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Reference to the event',
    `command`  ENUM ('START','PAUSE','ADD','REMOVE') NOT NULL COMMENT 'Type of the executed command',
    `seconds`  BIGINT COMMENT 'Seconds that the command potentially changed',
    PRIMARY KEY (`event_id`),
    CONSTRAINT `fk_command_event_event`
        FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
);

CREATE TABLE IF NOT EXISTS `timer_event`
(
    `id`                  BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Id of the timer event',
    `timer_id`            UUID COMMENT 'Reference to the timer',
    `channel_id`          VARCHAR(30) NOT NULL COMMENT 'Twitch id of the channel',
    `event_id`            BIGINT UNSIGNED COMMENT 'Reference to the subathon event',
    `timestamp`           TIMESTAMP NOT NULL COMMENT 'Timestamp of the timer event',
    `type`                ENUM ('TIME_ADDITION', 'TIME_SUBTRACTION', 'STATE_CHANGE') NOT NULL COMMENT 'Type of the timer event',
    `old_end_time`        TIMESTAMP COMMENT 'End timestamp before timer event',
    `current_end_time`    TIMESTAMP COMMENT 'End timestamp after timer event',
    `old_timer_state`     ENUM ('UNINITIALIZED', 'INITIALIZED', 'PAUSED', 'TICKING', 'ENDED') NOT NULL COMMENT 'Timer state before the timer event',
    `current_timer_state` ENUM ('UNINITIALIZED', 'INITIALIZED', 'PAUSED', 'TICKING', 'ENDED') NOT NULL COMMENT 'Timer state after the timer event',
    `insert_time`         TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT 'Insertion timestamp, generated by database',
    PRIMARY KEY (`id`),
    CONSTRAINT `fk_timer_event_event`
        FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,
    CONSTRAINT `fk_timer_event_timer`
        FOREIGN KEY (`timer_id`) REFERENCES `timer` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
);